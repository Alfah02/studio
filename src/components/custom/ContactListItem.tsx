    ...
    <Card className="hover:shadow-lg transition-shadow duration-200">
      <CardContent className="p-4 flex items-center space-x-4">
        ...
        <div className="flex items-center space-x-1 sm:space-x-2">
          <Button variant="ghost" size="icon" onClick={() => onToggleFavorite(contact.id)} aria-label={contact.isFavorite ? "Unfavorite" : "Favorite"}>
            <Star className={cn("h-5 w-5", contact.isFavorite ? "fill-yellow-400 text-yellow-500" : "text-muted-foreground hover:text-yellow-500")} />
          </Button>
          <Button variant="outline" size="icon" onClick={() => onCall(contact, 'audio')} aria-label={`Audio call ${contact.name}`}>
            <Phone className="h-5 w-5 text-green-500" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => onCall(contact, 'video')} aria-label={`Video call ${contact.name}`}>
            <Video className="h-5 w-5 text-blue-500" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => onEdit(contact)} aria-label={`Edit ${contact.name}`}>
            <Edit3 className="h-5 w-5 text-muted-foreground hover:text-primary" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => onDelete(contact.id)} aria-label={`Delete ${contact.name}`}>
