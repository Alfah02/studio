...
const generateImageFlow = ai.defineFlow(
  ...
  async (input) => {
    const {media} = await ai.generate({
      ...
      prompt: `Generate an image for a tech application landing page: ${input.prompt}. The image should be modern, clean, and visually appealing. Aspect ratio 16:9 or similar suitable for a card.`,
      config: {
